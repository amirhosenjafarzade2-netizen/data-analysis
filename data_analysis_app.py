...
with st.sidebar.expander("ℹ️ User Guide"):
    ...
    - **Automated EDA**: Custom comprehensive report with visuals (built-in, no external deps).
...
with st.expander("🧹 Data Cleaning Assistant"):
    ...
    if st.button("Apply Suggested Cleaning"):
        try:
            df_clean = apply_data_cleaning(df, cleaning_suggestions)
            st.session_state.df = df_clean
            st.success("✅ Data cleaning applied successfully!")
            st.dataframe(format_dataframe_for_display(df_clean.head(10)), use_container_width=True)
        except Exception as e:
...
analysis_options = {
    ...
    "eda_report": "Automated EDA Report (Custom Built-in)",
